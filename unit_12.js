...
function f12() {
    ...
    for (i = 0; i < a12.length; i++) {
        for (k = 0; k < a12[i].length; k++) {
            if (a12[i][k] == 1) {
                document.querySelector('.out-12').innerHTML += a12[i][k] + ' ';
            }
        }
    }
}
...
function f15() {
    ...
    for (i = 0; i < a15.length; i++) {
        if (a15[i].length > maxArr) maxArr += a15[i].length;
    }
    document.querySelector('.out-15').innerHTML = maxArr
}
